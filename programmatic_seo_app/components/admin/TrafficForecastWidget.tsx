  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100 mt-6">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-bold text-gray-800">? AI Predictive SEO Forecast (14-Days)</h3>
        <span className="px-2 py-1 bg-purple-100 text-purple-700 text-xs font-semibold rounded-full">BigQuery ML ARIMA_PLUS</span>
      </div>
      <p className="text-sm text-gray-500 mb-4">Expected traffic based on historical patterns.</p>
      
      <div className="space-y-3">
        {forecast.length > 0 ? forecast.map((f, idx) => (
          <div key={idx} className="flex justify-between items-center p-3 hover:bg-gray-50 rounded border border-transparent hover:border-gray-100 transition-colors">
            <div className="text-sm font-medium text-gray-700 truncate w-3/4">{f.url}</div>
            <div className="text-sm font-bold \">
              ~{f.clicks.toLocaleString()} clicks
            </div>
          </div>
        )) : (
          <div className="text-sm text-gray-500 italic">No prediction data available yet. Training requires more history.</div>
        )}
      </div>
    </div>
  );
