...
SUPABASE_URL = "https://idlmeduwekczlizgpvcx.supabase.co"
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImlkbG1lZHV3ZWtjemxpemdwdmN4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODY1NTUxODQsImV4cCI6MjEwMjEzMTE4NH0.oEULTKL9tE94c6vNp8vZtHGzQG0CFZG9nrHDuER9jvo"

locations = [
    "Greater Noida", "Noida", "Delhi", "Gurgaon", "Ghaziabad", "Faridabad",
    "Greater Kailash Delhi", "South Extension Delhi", "Safdarjung Enclave Delhi",
    "Hauz Khas Delhi", "Vasant Vihar Delhi", "Vasant Kunj Delhi", "Defense Colony Delhi",
    "New Friends Colony Delhi", "Saket Delhi", "Dwarka Sector 11 Delhi", "Dwarka Sector 12 Delhi",
    "Sector 15 Noida", "Sector 62 Noida", "Sector 18 Noida", "Indirapuram Ghaziabad", "Raj Nagar Ghaziabad"
]
...
services = [
    ...
    },
    {
        ...
        "meta": "Certified physiotherapy at home in {LOC}. Post-surgery rehabilitation, stroke recovery, paralyis rehab & joint pain relief by Stoic Home Care physios.",
...
for loc in locations:
    for srv in services:
        slug = f"{srv['id']}-at-home-{loc.lower().replace(' ', '-')}"
        seo_payload.append({
            "slug": slug,
            "category": srv["category"],
            "location": loc,
            "page_title": f"Best {srv['title']} in {loc} | Stoic Home Care",
            "meta_desc": srv["meta"].replace("{LOC}", loc),
            "h1_title": f"Expert {srv['title']} in {loc}",
            "hero_subtitle": srv["hero"].replace("{LOC}", loc),
            "content_html": srv["content"].replace("{LOC}", loc)
        })
...
def send_to_supabase(table, data):
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            print(f"✅ Seeded {len(data)} items into {table} (HTTP {resp.status})")
    except Exception as e:
        print(f"❌ Error seeding {table}: {e}")
        if hasattr(e, 'read'):
            print(e.read().decode('utf-8'))

send_to_supabase("stoic_home_care", seo_payload)
send_to_supabase("stoic_blogs", blogs_payload)
